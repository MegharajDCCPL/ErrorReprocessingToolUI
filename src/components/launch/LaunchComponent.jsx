...
const LaunchComponent = () => {
  ...
  const handleServerSelection = (selected) => {
    try {
  ...
  const handleLaunchClick = async (event) => {
    ...
    const apiUrl = `${ERT_API_URLS.SubAppAccess_URL}`;
    try {
      const response = await ApiMethods.handleApiPostSimple(
        payload,
        apiUrl,
        `JSON Data successfully.`,
        setLoading,
        retryCount,
        uuid,
        setUuid
